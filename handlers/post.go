...
func PostHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err == nil {
		...
		if err == sql.ErrNoRows {
			...
			RenderError(w, "Database Error", http.StatusInternalServerError)
			return
		}
	}
	...
	if r.Method == http.MethodPost {
		...
		if err != nil {
			RenderError(w, "Error creating post", http.StatusInternalServerError)
			return
		}

		postID, err := result.LastInsertId()
		if err != nil {
			RenderError(w, "Error retrieving post ID", http.StatusInternalServerError)
			return
		}
		...
		for _, category := range categories {
			...
			if err != nil {
				RenderError(w, "Error inserting categories", http.StatusInternalServerError)
				return
			}
		}
